<div class="overlay_demo_page"></div>

<div class="lui_phone_navbar_overlay opened" data-target="search">
	<div class="search">
		<form action="">
			<input type="text" placeholder="Enter search term" />
			<button class="lui_button secondary fa_b_search"></button>
		</form>

		<div class="results">
			<div class="found">649 results found</div>
			<div class="actions">
				<div class="lui_button primary">View results</div>
			</div>
		</div>
	</div>
</div>


<div class="lui_phone_navbar">
	<a data-target="categories" style="pointer-events:none;">
		<div class="fa_b_navicon">Menu</div>
	</a>
	<a class="active" data-target="search">
		<div class="fa_b_search">Search</div>
	</a>
	<a style="pointer-events:none;">
		<div class="fa_b_home">Home</div>
	</a>
	<a data-target="user" style="pointer-events:none;">
		<div class="fa_b_user">User</div>
	</a>
	<a data-target="basket" style="pointer-events:none;">
		<div class="fa_b_shopping-cart">Basket</div>
	</a>
</div>

<!-- SCRIPTS FOR DEMO -->
<script src="/less/lui3/LUI-3_phone-navbar/js/lui.phone-navbar.js"></script>