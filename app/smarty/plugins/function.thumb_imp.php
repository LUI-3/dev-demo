...
function smarty_function_thumb_imp($params, &$smarty) {/*{{{*/
	...
	### check parameters/*{{{*/
		if (empty($params['cache'])) {
          $_CONFIG['cache'] = $_SERVER["DOCUMENT_ROOT"] . '/data/cache/img/';
          $_CONFIG['cache_no_root'] = $_SERVER["DOCUMENT_ROOT"]  . "/data/cache/img/";
        } else {
          $_CONFIG['cache'] = $params['cache'];
        }
        if ((substr($_CONFIG['cache'], -1)) != '/') $_CONFIG['cache'] .= "/";

	$params['file_route'] = $_SERVER["DOCUMENT_ROOT"] . $params['file'];


	if (empty($params['file_route'])) { echo "<br />file empty"; }
	if (!file_exists($params['file_route'])) { echo "<br />image doesnt exists"; }
	...
	$temp = getimagesize($params['file_route']);
	
	$_FILE['output_url'] = $params['file'];

	

	$_SRC['file']		= $params['file_route'];	
	$_SRC['width']		= $temp[0];
	$_SRC['height']		= $temp[1];
	$_SRC['type']		= $temp[2]; // 1=GIF, 2=JPG, 3=PNG, SWF=4
	$_SRC['string']		= $temp[3];
	$_SRC['filename'] 	= basename($params['file_route']);
	$_SRC['modified'] 	= filemtime($params['file_route']);

	$sub_cache_dir_name = date("Y_m_d",filemtime($params['file_route']));
	$sub_cache_dir = $_SERVER["DOCUMENT_ROOT"] . "/data/cache/img/".$sub_cache_dir_name;
	if(!file_exists($sub_cache_dir))
		mkdir($sub_cache_dir."/");
	...
	if (is_numeric(isset($params['longside']))) {
    ...
    $_DST['file'] = $_CONFIG['cache'].$sub_cache_dir_name."/".$tmp_filename."_".$_SRC['hash'].$_CONFIG['types'][$_DST['type']];
    $_DST['file_no_root'] = $_CONFIG['cache_no_root'].$sub_cache_dir_name."/".$tmp_filename."_".$_SRC['hash'].$_CONFIG['types'][$_DST['type']];


	$params['getcachefilename'] = $tmp_filename."_".$_SRC['hash'].$_CONFIG['types'][$_DST['type']];
	...
	$params['img_out_way'] = "/data/cache/img/" . $sub_cache_dir_name . "/" . $params['getcachefilename'];
	$params['getcachefilename'] = $params['img_out_way'];
	if (empty($params['html'])) $_RETURN['img'] = '<img src="'.$params['img_out_way'].'" '.$_DST['string'].' alt="" title="" />';
	else $_RETURN['img'] = '<img src="'.$params['img_out_way'].'" '.$params['html'].' '.$_DST['string'].' />';
	if(!isset($params['html']))
		$params['html'] = null;

	$_RETURN['img_original_ver'] = '<img src="'.$params['img_out_way'].'" '.$params['html'].' '.$_DST['string'].' />';
	...
	if (file_exists($_DST['file_no_root']) OR (isset($params['dev']) && !$params['dev'])) {
        if (!$params['getcachefilename']) {
            //return normal html image string
            return $_RETURN['img_original_ver'];
        } else {
